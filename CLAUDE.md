...
- `static/css/lintashome.css` is the active site stylesheet because `data/settings.yaml` sets `color_name: lintashome`. It imports the theme CSS files and then applies Lintashomes-specific overrides.
...
- Keep Lintashomes-specific CSS in `static/css/lintashome.css` unless changing reusable theme behavior. Theme base styles are imported from `/css/fonts.css`, `/css/base.css`, `/css/header-footer.css`, and `/css/bricks.css`.
